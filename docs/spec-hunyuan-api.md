...
### 3.1 计费与精度档位（2026-09-03 核实）

官方 1804 计费表（专业版，默认 20 积分/次）。**生成失败不扣积分**（含风控）。

**基础档（`GenerateType`，必选其一，默认 Normal）：**
...
| 参数 | 积分 | 说明 |
|---|---|---|
| `MultiViewImages` | +10 | 多视图 |
| `EnablePBR` | +10 | PBR（金属度/粗糙度/法线） |
| `FaceCount` | +10 | 自定义面数 3000–1.5M（与 LowPoly 互斥） |
| `ResultFormat` | +5 | 生成格式 |

> **精度档位结论（对齐 2026-09-03 决策）：**
> - **降面不省钱。** `FaceCount` 是**固定 +10**，不随面数多少变化。想"低面数低成本"在本接口不成立。
> - **真正的低成本档**是 `Geometry`(15,白模) 或 `Normal`(20) **不开 PBR**（省 10 分）。
> - **尽量用 3.1。** 3.1 与 3.0 基础价相同（同一档 20 分），但几何/纹理更好、支持八视图。产品要求质量好，故主路径固定 Model=3.1。
> - **面数/轻量化交给后处理。** 混元结果约 50 万面（40+ MB）。如需低面数，用 `trimesh` 等**下载后**做减面/简化，而非靠接口参数（那要加价）。相关技术见 `docs/spec-optimize.md`。
